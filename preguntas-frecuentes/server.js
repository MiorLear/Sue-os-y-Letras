...
// Busca la FAQ más relevante comparando palabras clave de la
// pregunta del usuario contra las preguntas y respuestas guardadas.
// Sin APIs externas, sin costo, funciona completamente offline.
...
function calcularPuntaje(preguntaUsuario, faq) {
  const palabrasUsuario = normalizarTexto(preguntaUsuario).split(/\s+/).filter((p) => p.length > 2);
  const textoFaq = normalizarTexto(`${faq.pregunta} ${faq.respuesta}`);

  let puntaje = 0;
  for (const palabra of palabrasUsuario) {
    if (textoFaq.includes(palabra)) puntaje++;
  }
  return puntaje;
}

function buscarRespuesta(preguntaUsuario, faqs) {
  // Calculamos puntaje para cada FAQ
  const resultados = faqs
    .map((faq) => ({ faq, puntaje: calcularPuntaje(preguntaUsuario, faq) }))
...
// Respuestas cuando no se encuentra nada relacionado
...
async function handleApi(request, response) {
  ...
  if (request.method === "GET" && request.url === "/api/faqs") {
    ...
    sendJson(response, 200, { ok: true, faqs });
    return;
  }

  // POST /api/chat — busca la mejor respuesta para la pregunta
  if (request.method === "POST" && request.url === "/api/chat") {
    ...
    if (body.pregunta.trim().length > 500) {
      sendError(response, 422, "La pregunta es demasiado larga. Máximo 500 caracteres.");
      return;
    }
    ...
    let respuesta;
    if (resultado) {
      // Encontramos una FAQ relevante — respondemos con su contenido
      respuesta = resultado.respuesta;
    } else {
      // Ninguna FAQ coincide — damos una respuesta genérica de fallback
      respuesta = respuestaAleatoria(respuestasNoEncontrado);
    }

    sendJson(response, 200, { ok: true, respuesta });
    return;
  }
...
const server = http.createServer(async (request, response) => {
  try {
    ...
    if (!served) sendError(response, 404, "Página no encontrada.");
  } catch (error) {
...
server.listen(PORT, () => {
  console.log(`Preguntas Frecuentes — ExplorArte corriendo en http://localhost:${PORT}`);
});
